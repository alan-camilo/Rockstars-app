...
class RockstarRepository(val dao: RockstarDao, val app: Application) {
        ...
        if (rockstars.size > 0) {
            ...
            RockstarApiClient().service.listRockstars().enqueue(object : Callback<ArrayList<Rockstar>> {
                override fun onFailure(call: Call<ArrayList<Rockstar>>, t: Throwable) {
                    ...
                    //http request failed, so retrieve what's in the db
                    GlobalScope.launch(Dispatchers.IO) {
                        val listFromDB = dao.getAll().map { it.toRockstar() }
                        withContext(Dispatchers.Main) { list.value = ArrayList(listFromDB) }
                    }
                }
